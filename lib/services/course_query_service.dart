...
class CourseQueryService {
  ...
  Future<Map<String, dynamic>> getSemesters() async {
    ...
    try {
      ...
      return jsonDecode(vRes.body) as Map<String, dynamic>;
    } finally {
      client.close();
    }
  }
  ...
  // 取得資料 (三階段請求)
  Future<List<CourseJsonData>> getCourses({
    String? semester,
    bool forceRefresh = false,
  }) async {
    if (_isDataLoaded &&
        !forceRefresh &&
        _cachedCourses.isNotEmpty &&
        (semester == null || semester == _currentSemester)) {
    ...
    try {
      ...
      if (targetSem.isEmpty) {
        ...
        targetSem = vJson['latest'];
      }
      _currentSemester = targetSem;
      // debugPrint("   -> 最新學期: $targetSem");

      // 2. 取得該學期的 latest time
      final tRes = await client.get(
        Uri.parse(
          "https://nsysu-opendev.github.io/NSYSUCourseAPI/$targetSem/version.json",
        ),
      );
      if (tRes.statusCode != 200) throw "Time API Error";
      final tJson = jsonDecode(tRes.body);
      final String latestTime = tJson['latest'];
      // debugPrint("   -> 最新時間戳: $latestTime");

      // 3. 取得 all.json
      final url =
          "https://nsysu-opendev.github.io/NSYSUCourseAPI/$targetSem/$latestTime/all.json";
      // debugPrint("   -> 下載大檔: $url");
      final allRes = await client.get(Uri.parse(url));
      if (allRes.statusCode != 200) throw "All JSON API Error";

      // 解析 List 與對應物件 (在 Isolate 背景執行)
      _cachedCourses = await Isolate.run(() {
        final List<dynamic> rawList = jsonDecode(utf8.decode(allRes.bodyBytes));
        return rawList.map((e) => CourseJsonData.fromJson(e)).toList();
      });
      _isDataLoaded = true;

      // debugPrint("✅ [課程API] 資料載入完成，共 ${_cachedCourses.length} 筆");
      return _cachedCourses;
    } catch (e) {
      debugPrint("❌ [課程API] 錯誤: $e");
      throw e;
    } finally {
      client.close();
    }
    ...
    return _cachedCourses
        .where((course) {
          ...
          if (code != null && code.isNotEmpty) {
            if (!course.id.toUpperCase().contains(code.toUpperCase()))
              return false;
          }
          // 4. 年級 (精確)
