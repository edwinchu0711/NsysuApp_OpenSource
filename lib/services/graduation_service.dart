...
class GraduationService {
  ...
  static const String _cacheKeyData = 'grad_data_cache';
  ...
  /// 獲取資料 (自動判斷是否使用快取)
  /// 回傳 GraduationData? 以防萬一出錯時可以 handle
  Future<GraduationData?> fetchGraduationData({
    ...
    if (!forceRefresh) {
      ...
      if (lastTime != null) {
        ...
        if (diff.inMinutes < 10) {
          final String? jsonStr = prefs.getString(_cacheKeyData);
    ...
    // 2. 從 Preferences 讀取帳密 (你的 main menu 應該已經存好了)
  ...
  Future<GraduationData> _fetchFromNetwork(
    String username,
    String password,
  ) async {
    ...
    try {
      ...
      final http.Request loginRequest =
          http.Request(
              'POST',
              Uri.parse(
      ...
      // Step D: 寫入快取
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(_cacheKeyData, jsonEncode(data.toJson()));
  ...
  GraduationData _parseHtml(String html) {
    ...
    var infoMatch = infoReg.firstMatch(
      fullText.replaceAll('\n', '').replaceAll('\r', ''),
    );
    if (infoMatch != null) {
    ...
    for (var tr in trs) {
      ...
      if (text.contains("學系必修課程缺修")) {
        ...
        inGenEdSection = false;
        inElectiveSection = false;
        continue;
      }
        ...
        if (isMainStatusRow) {
          ...
          for (var td in tds) {
            ...
            if (t.contains("缺") || t.contains("尚缺")) {
              ...
              if (itemName ==
                  "※注意：以上各類別之「檢核欄」必須全部為「符合」且通識課程必修至少２８學分，方達成所有通識課程畢業要求。") {
                itemName = "通識28學分要求";
              }
            }
          }
