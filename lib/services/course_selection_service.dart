...
class CourseSelectionService {
  ...
  Future<Map<String, dynamic>> fetchSelectionResult() async {
    ...
    try {
      ...
      // 嘗試從 CourseQueryService.instance.getSemesters() 取得最新學期作為 fallback
      String? fallbackSem;
      try {
        final semData = await CourseQueryService.instance.getSemesters();
        fallbackSem = semData['latest'] as String;
      } catch (e) {
        debugPrint("⚠️ 無法取得最新學期 fallback: $e");
      }
  ...
  List<CourseSelectionData> _parseSelectionTable(
    ...
    String? currentParsedSemester;
    if (semMatch != null) {
      final year = semMatch.group(1);
      var term = semMatch.group(2);
      if (term == '一') term = '1';
      if (term == '二') term = '2';
      currentParsedSemester = "$year$term";
      debugPrint("🔍 [解析] 偵測到選課網頁之學期: $currentParsedSemester");
    }
    final semesterCode = currentParsedSemester ?? fallbackSemester;
