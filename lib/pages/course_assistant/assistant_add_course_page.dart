...
class AssistantAddCoursePage extends StatefulWidget {
  const AssistantAddCoursePage({Key? key}) : super(key: key);
...
class _AssistantAddCoursePageState extends State<AssistantAddCoursePage> {
  ...
  Future<void> _loadExistingAssistantCourses() async {
    try {
      ...
      if (jsonStr != null && jsonStr.isNotEmpty) {
        ...
        setState(() {
          _existingAssistantCourseIds = decoded.map((v) => v['code'].toString()).toSet();
        });
      }
    } catch (e) {
      print("讀取既有助手課表失敗: $e");
  ...
  Future<void> _addCourseToAssistant(CourseJsonData courseData) async {
    try {
      ...
      if (currentList.any((c) => c['code'] == courseData.id)) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("此課程已經在助手中了！")));
        return;
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("已排入模擬課表：${courseData.name.split('\n')[0]}"), backgroundColor: Colors.green),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("加入失敗：$e")));
    }
  }
  ...
  Widget _buildSearchResults() {
    ...
    if (!_hasSearched) {
      return Center(child: Text("點擊上方按鈕搜尋想加入的課程", style: TextStyle(color: Colors.grey[400])));
    }
  ...
  Future<void> _performSearch() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("搜尋失敗或資料載入錯誤: $e")));
      }
    }
  }
